...
import { UploadCloud, X, Loader2, Send, Sparkles } from "lucide-react";
import { useMutation } from "@tanstack/react-query";
import { showSuccess, showError } from "@/shared/utils/toast";
import { clsx, type ClassValue } from "clsx";
import { twMerge } from "tailwind-merge";
import { motion } from "framer-motion";
import { createStory } from "@/shared/api/story.api";
...
export default function UploadSelfie() {
  ...
  const mutation = useMutation({
    mutationFn: ({ image, caption }: { image: File; caption: string }) =>
      createStory({ image, caption }),
    onSuccess: () => {
      showSuccess("Selfie posted successfully!");
      clearSelection();
      setCaption("");
    },
    onError: (err: unknown) => {
  ...
  const handleUpload = async () => {
    if (!file) return;
    mutation.mutate({ image: file, caption });
  };
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-zinc-400">
              Upload and share your latest selfie with the world.
            </p>
          </div>
        </div>
        ...
        {!previewUrl ? (
          ...
          >
            ...
            <div className="flex flex-col items-center space-y-4">
              ...
              <button className="px-4 py-2 text-xs font-semibold tracking-wide text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40 rounded-xl hover:bg-indigo-100/50 transition-colors pointer-events-none">
                Browse File
              </button>
            </div>
          </div>
