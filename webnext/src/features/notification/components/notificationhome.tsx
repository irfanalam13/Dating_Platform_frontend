...
import { useNotifications, useMarkNotificationRead } from "@/features/notification/hooks/useNotifications";
  ...
  return (
    <main className="min-h-[100dvh] bg-[#FFF8F1] px-4 pb-24 pt-5 text-[#2D2424]">
      <div className="mx-auto max-w-md">
        ...
        {isLoading && (
          <div className="rounded-lg border border-[#EADDD2] bg-white p-5 text-sm text-[#746767]">
          ...
          {data.map((item) => (
            ...
            >
              <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-full ${
                item.is_read ? "bg-[#F8EFE6] text-[#746767]" : "bg-[#7A2432] text-white"
              }`}>
                <NotificationIcon type={item.notification_type} />
              </span>
              <span className="min-w-0 flex-1">
                <span className="block font-semibold">{item.title}</span>
                <span className="mt-1 block text-sm leading-6 text-[#746767]">
